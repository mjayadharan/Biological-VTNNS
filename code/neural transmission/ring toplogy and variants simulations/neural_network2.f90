program neural_network1
  ...
  real(8),parameter:: weight_plus=1.2,weight_plus_plus=1.6, weight_minus=-1.2
  real(8),parameter:: adjuster=0.01 , time_lag=0.001
  ...
  real(8):: next_fire_time,printing_array(num+1),update_time_table(num),update_time
...
!   weight_matrix= weight_plus
 !   weight_matrix(num,1)=weight_minus
  do l =1,num
 ...
 !   end do
    !   close(3)
    write(7,*) neural_network(3,:)
    update_time_table=100000.0
       ...
       if(update_time<next_fire_time) then
          ...
          do l =1,num
             if(firing_table(l)==next_fire_time) then
                ...
                write(1,*) 20.0* next_fire_time,l,0.5
