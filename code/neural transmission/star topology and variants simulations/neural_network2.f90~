program neural_network1
  ...
  real(8),parameter:: time_bound= 5.0
  real(8) , parameter :: tau=20.0, tau_s=5.0/tau
  real(8),parameter:: E_plus = 74.0, E_minus= -6.0
  real(8),parameter:: weight_plus=0.6, weight_minus=-0.6
  real(8),parameter:: adjuster=0.01 , time_lag=0.01
...
!      weight_matrix(l+1,1)= weight_plus
 ...
 !   weight_matrix(num,1)=weight_plus
  do l =1,num
    ...
    neural_network(l,1)= 12.0- 9.0*random1
 end do
...
if(random_dummy>0) then
          ...
          !        write(*,*) "curve error"
              ...
              firing_table(k)=firing_time + guess
           end if
        end if
     end if
  end do
