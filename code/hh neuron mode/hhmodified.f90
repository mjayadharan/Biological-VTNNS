program hhmodified
 ...
 !membrane current is to be simulated with change in the gating variables
  ...
  b_h= beta_h(voltage)
     ...
     call euler_four(potassium_open,0.0001,initial_pp,final_pp,n_initial_k,n_final_k,a_n,b_n)
     !write(*,*) initial_pp, final_pp
     i_k=G_K*(n_final_k**4)*(voltage-V_k) !potassium current


     
     call euler_four(sodium_open,0.0001,initial_pp,final_pp,m_initial,m_final,a_m,b_m)
     call euler_four(sodium_close,0.0001,initial_pp,final_pp,h_initial,h_final,a_h,b_h)
 ...
 dv= voltage_function(step_s,voltage,I_m,n_initial_k,m_initial,h_initial)    
